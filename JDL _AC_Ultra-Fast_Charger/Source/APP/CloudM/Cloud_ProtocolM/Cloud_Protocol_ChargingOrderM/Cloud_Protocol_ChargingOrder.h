...
typedef enum
{
    ...
    CLOUD_PROTOCOL_ORDER_STATUS_STARTED = 1,  // started
    CLOUD_PROTOCOL_ORDER_STATUS_CHARGING = 2, // charging
    CLOUD_PROTOCOL_ORDER_STATUS_FINISHED = 3, // finished
    CLOUD_PROTOCOL_ORDER_STATUS_UPLOADED = 4  // uploaded
...
typedef struct
{
    bool tsdb_synced;                              // Whether synchronized to TSDB
    uint32_t tsdb_id;                              // TSDB record ID
    time_t create_timestamp;                       // creation timestamp(For internal computing only)
    time_t update_timestamp;                       // update timestamp(For internal computing only)
...
extern bool cloud_protocol_charging_order_set_energy_interface(const cloud_protocol_energy_interface_t *interface);
extern cloud_protocol_charging_cloud_protocol_order_manager_t *cloud_protocol_get_active_charging_order_(uint8_t gun_no);
extern bool cloud_protocol_charging_order_is_gun_busy(uint8_t gun_no);
extern cloud_protocol_order_status_t cloud_protocol_charging_order_get_status(uint8_t gun_no);
extern cloud_protocol_charging_cloud_protocol_order_manager_t *cloud_protocol_charging_order_create(uint8_t gun_no, cloud_protocol_order_type_t type,
                                        cloud_protocol_transaction_type_t trans_type,
                                        const uint8_t *platform_transaction_id,
                                        const char *vin, const uint8_t *card);
extern void cloud_protocol_charging_order_release(uint8_t gun_no);
extern bool cloud_protocol_charging_order_start(uint8_t gun_no);
extern bool cloud_protocol_charging_order_update_all(void);
extern bool cloud_protocol_charging_order_finish(uint8_t gun_no, cloud_protocol_StopCharging_Type_E reason);
extern bool cloud_protocol_charging_order_stop_all(cloud_protocol_StopCharging_Type_E reason);
