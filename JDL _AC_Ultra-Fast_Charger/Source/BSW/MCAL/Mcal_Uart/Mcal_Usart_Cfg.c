...
|******************************************************************************/
static void Mcal_Usart_Enable_IT(uint32_t USART);
static void Mcal_Usart_Disable_IT(uint32_t USART);
 /* 4G */
...
void Mcal_Usart_Enable(void)
{
  ...
  for (i = 0; i < MCAL_USART_MAX_NUMBER; i++)
  {
     __HAL_UART_ENABLE(McalUsart_NumMapUsart[i].UsartBase);
    Mcal_Usart_Enable_IT(i);
  }
}
...
void Mcal_Usart_Disable(void)
{
  ...
  for (i = 0; i < MCAL_USART_MAX_NUMBER; i++)
  {
     __HAL_UART_DISABLE(McalUsart_NumMapUsart[i].UsartBase);
    Mcal_Usart_Disable_IT(i);
  }
}
...
static void Mcal_Usart_Enable_IT(uint32_t USART)
{
  ...
  // HAL_UARTEx_ReceiveToIdle_DMA(McalUsart_NumMapUsart[MCAL_USART1_CH].UsartBase, McalUsart_Ctrl[MCAL_USART1_CH].RcvIntSwapBuf[McalUsart_Ctrl[MCAL_USART1_CH].RcvIntSwapBufIdx], McalUsart_BufferCfg[USART].RcvCycBufLen);
  HAL_UARTEx_ReceiveToIdle_IT(McalUsart_NumMapUsart[USART].UsartBase, McalUsart_Ctrl[USART].RcvIntSwapBuf[McalUsart_Ctrl[USART].RcvIntSwapBufIdx], MCAL_USART_RCV_CYCBUF_MAX_LEN);
}
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART1)
  {
    ...
    McalUsart_Ctrl[MCAL_USART1_CH].Send_Lock = 0;                                                 // 发送完成，解锁
    McalUsart_Ctrl[MCAL_USART1_CH].SenLen = 0;                                                       // 清空发送长度
    memset(McalUsart_Ctrl[MCAL_USART1_CH].SendBuf, 0, McalUsart_Ctrl[MCAL_USART1_CH].SendBufLen); // 清空发送缓冲区
  }
  else if (huart->Instance == USART2)
  {
    __HAL_UART_CLEAR_FLAG(&huart2, UART_FLAG_TC);                                                 // 清除发送完成标志
    McalUsart_Ctrl[MCAL_USART2_CH].Send_Lock = 0;                                                 // 发送完成，解锁
    McalUsart_Ctrl[MCAL_USART2_CH].SenLen = 0;                                                       // 清空发送长度
    memset(McalUsart_Ctrl[MCAL_USART2_CH].SendBuf, 0, McalUsart_Ctrl[MCAL_USART2_CH].SendBufLen); // 清空发送缓冲区
  }
...
{
  for (uint32_t id = MCAL_USART1_CH; id < MCAL_USART_MAX_NUMBER; id++)
  {
      ...
      if (MCAL_CYCBUF_RET_SUCCESS == MCAL_CYCBUF_CHECK_DATA(McalUsart_Ctrl[i].SendCycBufID, &McalUsart_Ctrl[i].SenLen))
      {
        if (McalUsart_Ctrl[i].SenLen > 0)
        {
          if (MCAL_CYCBUF_RET_SUCCESS == MCAL_CYCBUF_PREVIEW_READ(McalUsart_Ctrl[i].SendCycBufID, McalUsart_Ctrl[i].SendBuf, McalUsart_Ctrl[i].SenLen))
          {
            ...
            {
              // __HAL_UART_CLEAR_FLAG(McalUsart_NumMapUsart[i].UsartBase, UART_FLAG_TC); // 清除发送完成标志
              __HAL_UART_ENABLE_IT(McalUsart_NumMapUsart[i].UsartBase, UART_IT_TC);    // 使能发送完成中断
              ...
              McalUsart_Ctrl[i].Send_Lock = 1; // 锁定发送
            }
          }
        }
        else
